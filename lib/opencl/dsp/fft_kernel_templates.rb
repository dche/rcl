...
module OpenCL
  module DSP
    module FFTKernelTemplates
      ...
      def kernel(name, max_radix, lmem_size, body)
        args = if self.data_format == :split_complex
          '__global float *in_real, __global float *in_image, __global float *out_real, __global float *out_image, int dir, int S'
        else
...
<%= body %>
      ...
      def formatted_load(aIndex, gIndex)
        if self.data_format == :interleaved_complex
          ...
          "a[#{aIndex}].x = in_real[#{gIndex}]; a[#{aIndex}].y = in_image[#{gIndex}];\n"
        end
      end
      ...
      def formatted_store(aIndex, gIndex)
        if self.data_format == :interleaved_complex
          ...
          "out_real[#{gIndex}] = a[#{aIndex}].x; out_image[#{gIndex}] = a[#{aIndex}].y;\n"
        end
      end
...
<% end %>
      ...
      def local_fft_kernel_string(n, radix_array)
        ...
        lmem_size = global_stores_and_transpose(body, n, max_radix,
                                               ...
                                               numXFormsPerWG)
        kernel[:lmem_size] = [kernel[:lmem_size], lmem_size].max
        kernel[:source] = kernel(kernel[:name], max_radix, kernel[:lmem_size], body)
        @kernels << kernel
      end
      ...
      def global_fft_kernel_string(n, bs, axis, vert_bs)
        ...
        pass_number.times do |i|
          ...
          strideO = r_init * (i == 0 ? 1 : radixArray.take(i).reduce(:*))
...
<% if vertical %>
    ...
    i = tid >> <%= lgStrideO %>;
    j = tid & <%= strideO - 1 %>;
<% stride = radix * r_init * (i == 0 ? 1 : radixArray.take(i).reduce(:*)) %>
...
<% end %>
<% r1.times do |j| %>
    a[<%= j %>].y = in_image[<%= j * gInInc * strideI %>];
<% end %>
<% else %>
    in += indexIn;
<% r1.times do |j| %>
...
<% end %>
<% else %>
<% innerIter = radix / threadsPerBlock; outerIter = r1 / innerIter %>
<% outerIter.times do |k| %>
<% innerIter.times do |t| %>
    a[<%= k * innerIter + t %>].x = lMemLoad[<%= t * threadsPerBlock + k * (radix + 1) %>];
<% end %>
<% end %>
<% end %>
    barrier(CLK_LOCAL_MEM_FENCE);
...
<% outerIter.times do |k| %>
<% innerIter.times do |t| %>
    a[<%= k * innerIter + t %>].y = lMemLoad[<%= t * threadsPerBlock + k * (radix + 1) %>];
<% end %>
<% end %>
<% end %>
    barrier(CLK_LOCAL_MEM_FENCE);
